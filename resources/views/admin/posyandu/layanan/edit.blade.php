{{-- resources/views/admin/posyandu/layanan/edit.blade.php --}}
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-900">Form Edit Layanan Posyandu</h2>
    </div>

    <form action="{{ route('admin.posyandu.layanan.update', $layanan) }}" method="POST" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="nama_layanan" id="nama_layanan" required
                    ...
                    value="{{ old('nama_layanan', $layanan->nama_layanan) }}" placeholder="Contoh: Pemantauan Pertumbuhan">
                @error('nama_layanan')
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                placeholder="Deskripsi lengkap tentang layanan yang diberikan">{{ old('deskripsi', $layanan->deskripsi) }}</textarea>
            @error('deskripsi')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="jadwal" id="jadwal" required
                    ...
                    <option value="Bulanan" {{ old('jadwal', $layanan->jadwal) == 'Bulanan' ? 'selected' : '' }}>Bulanan</option>
                    <option value="Mingguan" {{ old('jadwal', $layanan->jadwal) == 'Mingguan' ? 'selected' : '' }}>Mingguan</option>
                    <option value="Sesuai jadwal" {{ old('jadwal', $layanan->jadwal) == 'Sesuai jadwal' ? 'selected' : '' }}>Sesuai jadwal</option>
                    <option value="Setiap kunjungan" {{ old('jadwal', $layanan->jadwal) == 'Setiap kunjungan' ? 'selected' : '' }}>Setiap kunjungan</option>
                    <option value="Sewaktu-waktu" {{ old('jadwal', $layanan->jadwal) == 'Sewaktu-waktu' ? 'selected' : '' }}>Sewaktu-waktu</option>
                </select>
                @error('jadwal')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="target_usia" id="target_usia" required
                    ...
                    <option value="0-5 tahun" {{ old('target_usia', $layanan->target_usia) == '0-5 tahun' ? 'selected' : '' }}>0-5 tahun</option>
                    <option value="0-2 tahun" {{ old('target_usia', $layanan->target_usia) == '0-2 tahun' ? 'selected' : '' }}>0-2 tahun</option>
                    <option value="1-5 tahun" {{ old('target_usia', $layanan->target_usia) == '1-5 tahun' ? 'selected' : '' }}>1-5 tahun</option>
                    <option value="Ibu & Balita" {{ old('target_usia', $layanan->target_usia) == 'Ibu & Balita' ? 'selected' : '' }}>Ibu & Balita</option>
                    <option value="Ibu Hamil" {{ old('target_usia', $layanan->target_usia) == 'Ibu Hamil' ? 'selected' : '' }}>Ibu Hamil</option>
                    <option value="Semua usia" {{ old('target_usia', $layanan->target_usia) == 'Semua usia' ? 'selected' : '' }}>Semua usia</option>
                </select>
                @error('target_usia')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="border-t pt-6">
            ...
            <div class="bg-gray-50 rounded-xl p-6 text-center">
                <div class="inline-flex items-center justify-center w-16 h-16 bg-green-100 rounded-full mb-4">
                    <div id="preview-icon" class="text-2xl">{{ $layanan->icon ?? '📋' }}</div>
                </div>
                <h3 id="preview-nama" class="text-lg font-bold text-gray-900 mb-2">{{ $layanan->nama_layanan }}</h3>
                <p id="preview-deskripsi" class="text-sm text-gray-600 mb-3">{{ $layanan->deskripsi }}</p>
                <div class="space-y-2">
                    <div class="flex items-center justify-center text-sm text-gray-500">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"></path>
                        </svg>
                        <span id="preview-jadwal">{{ $layanan->jadwal }}</span>
                    </div>
                    <div class="flex items-center justify-center text-sm text-gray-500">
                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path>
                        </svg>
                        <span id="preview-target">{{ $layanan->target_usia }}</span>
                    </div>
                </div>
            </div>
        </div>
